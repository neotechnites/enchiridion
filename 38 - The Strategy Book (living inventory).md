...
>
> **EVIDENCE CLASSES (Ryan's ruling, 2026-07-25):** Class A = full multi-year backtest exists
> and passes. Class B = history-capped new markets ("a lesser class, of course") — implementable
> but subordinate: smaller size, lower capital priority; hunting effort tilts toward Class A
> terrain (markets where deep backtests are constructible).
...
## COLUMN 3 — AWAITING FABLE TESTING (trial-test survivors; DEEP REVIEW SCHEDULED Sun 2026-07-26)
1. **H10** econ point-ladder maker (+1.5¢/fill implied, n=20,585) — also needs Ryan's live-probe approval.
2. **H9** political spread-capture maker (+0.5¢/fill, n=35,335, ≥2¢-spread gate) — same.
3. **SEED-PRIOR** listing harvest (+18.9¢ net, n=237 confident subset; fast-snap capture running).
4. **DOGE MAX ivol ramp** (~21¢ net top rungs; needs multi-issuance backtest — part of the review).
5. **SPOKEN-COUNT** (MUSK +39.5¢ family; vehicle-gated; Tuesday capture builds Warsh base rates).
6. **DUTCHBOOK** (real arbs, median 0.65¢ sub-slippage; detector fix + ≥1.5¢ + lifetime proof).
7. **INFLATION-FLASH** prelim→final (|Δ|≤0.1pp >90%; settlement-source access partly walled).
8. **FED-HOLD** (mechanism confirmed, n=1; forward capture accruing).
9. **ZERO-FEE SERIES** (13 series; liquidity-trigger watch).
10. **RENTEC widened** (window-limited PROMISING held).
