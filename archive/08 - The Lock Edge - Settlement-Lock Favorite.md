...
> 🔴 **DEAD (2026-08-06, FINAL VERDICT OF THAT DAY). Do not trade, do not build.** The July decay kill was retracted in the morning and the edge was re-killed the same evening on better evidence. The retracting re-scan carried a **60-second lookahead** (Coinbase candles labelled by bucket START, stored as bucket CLOSE, read forward) — and its exact reproduction of this note's control proved only that the control shared the defect. Corrected, 93–97¢ Z≥4 over 73 days goes **+3.08¢ → +0.84¢ (lookahead removed) → −0.38¢ (price-source error)**. Only XRP survives per-asset and not with margin. Separately, live orderbook capture shows the quote source we backtest on reads LOW by median +1.30¢ (p90 +8.0¢) in this band. **The rule below is not tradeable as written.** What SURVIVES from this note: the 93¢ hard floor (re-confirmed — the 90–93¢ "better band" was a price-labelling artifact), and the Z construction itself as a *risk* description. Full record: [[38 - The Strategy Book (living inventory)]] LOCK entry.

> ~~✅ THE 2026-07-23 DECAY KILL IS RETRACTED (2026-08-06). The edge measures ALIVE.~~ *(superseded within the day — kept so the sequence is legible: a numberless kill, a lookahead-driven revival, then a corrected death. Verdicts are dated; so are revivals.)* The kill cited +1.72¢/contract (first 6 wks) → −1.07¢ (last 4 wks). On a complete re-pull (34,459 markets, real `floor_strike` on all five assets, 2026-05-25→08-05) the kill's own "last 4 weeks" window measures **+3.39¢/contract pooled (n=752)**, and **every single week is positive, pooled and BTC-alone**. Control reproduced this note's numbers to the digit three ways, including a fresh independent pull. −1.07¢ is arithmetically unreachable — it needs a 4.44% flip rate; the measured Z≥4 flip rate is **0.181%** (90/49,747). What produced −1.07¢ is UNKNOWN; a data-dropout artifact was hypothesised and measured FALSE. **The rule below stands as written and is the correct rule** — with one exception: the "hard floor at 93¢" no longer holds (the 90–93¢ band is now the best cell, +6.4% at Z≥4), which needs its own test before the operating point moves. **This is not a green light to trade.** The one thing never measured is unchanged: real fills and depth at 93–97¢ in the final 2–4 min — unbacktestable, needs forward book capture. Everything here is last-trade + 0.5¢; at +3.3¢/contract, >3¢ of slippage kills it. Full record + the re-pull trap: [[38 - The Strategy Book (living inventory)]] LOCK entry.
