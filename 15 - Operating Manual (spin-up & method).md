...
## Execution defects (2026-07-25, after Ryan had to demand the retry fix)
**A missed fill is a DEFECT with a deadline, not data to admire.** When execution underperforms (orders not filling, entries late, edges unfetched), the report and the proposed fix arrive in the SAME message, with numbers — Ryan must never be the one to say "it should retry." Framing an execution failure as "honest data / the week's product" is the silver-lining violation in its execution form. Corollary from the same exchange: **an n-of-2 observation never overturns n-of-2yr research** — the Claude declared streak "a daytime strategy" off two quiet nights of asks; 8 days of tape showed overnight carries MORE signals at equal EV. Observations are labeled observations until they clear the note-07 bar.

**+EV left on the table is a DEFECT (2026-07-26, the 46¢-ceiling rebuke):** when evidence moves the EV frontier, the parameters move THE SAME DAY — under standing delegation Fable derives, implements, and reports; "ready on your word" is the violation when the word was already given. Ryan should never be the one to observe that a positive-EV bid beats no bid. ("why in the actual fuck do i need to be the one to realize the obvious fact… we should change our fucking strategy to maximize everything we can, and if we cant get 44, then get what we fucking can.")
